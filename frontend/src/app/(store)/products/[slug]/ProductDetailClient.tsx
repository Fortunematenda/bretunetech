...
export default function ProductDetailClient({ initialProduct }: { initialProduct: Product }) {
  ...
  // Client enhancements only — product body is server-rendered via initialProduct.
  useEffect(() => {
    const data = product;
    if (!data) return;
    ...
    try {
      ...
      localStorage.setItem(rvKey, JSON.stringify(filtered.slice(0, 10)));
    } catch (error) {
    ...
    (async () => {
      ...
      try {
        ...
        const nameWords = String(data.name || '')
          ...
          .replace(/[^a-z0-9\s]/g, ' ')
          .split(/\s+/)
          .filter((w) => w.length >= 3 && !stopWords.has(w) && !brandTokens.has(w) && !/^\d+$/.test(w));
        const productTypes = [
        ...
        const scoreCandidate = (p: any, base = 0) => {
          ...
          return { product: p, score: base + typeHits * 8 + featureHits * 4 + sameCategory + sameBrandPenalty };
        };
        ...
        for (const row of scored) {
          const pName = String(row.product.name || '').toLowerCase();
          if (!pName || usedNames.has(pName)) continue;
          const pBrand = String(row.product.brand?.name || '').toLowerCase();
          const isSameBrand = !!data.brand?.name && pBrand === String(data.brand.name).toLowerCase();
          if (isSameBrand && sameBrandCount >= 1) continue;
          if (isSameBrand) sameBrandCount += 1;
          usedNames.add(pName);
          final.push(row.product);
      ...
      <div className="mx-auto hidden w-full max-w-[1560px] px-6 py-8 lg:block">
        <nav className="mb-4 flex items-center gap-1.5 overflow-x-auto text-xs text-slate-500">
          ...
          {product.category?.parent && (
            <>
              <Link href={categoryPath(product.category.parent.slug)} className="whitespace-nowrap hover:text-[#003d7a]">
          ...
          {product.category && (
            <>
              <Link href={categoryPath(product.category.slug)} className="whitespace-nowrap hover:text-[#003d7a]">
