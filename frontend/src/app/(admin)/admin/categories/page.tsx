  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-white">Product Categories</h1>
      ...
      {showForm && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setShowForm(false)}>
          <div className="bg-[#1a1d27] border border-slate-700 rounded-2xl w-full max-w-md p-6 space-y-4" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between">
              <h2 className="text-base font-semibold text-white">{editItem ? 'Edit Category' : 'Add Category'}</h2>
          ...
          <div className="bg-[#1a1d27] border border-slate-700 rounded-2xl w-full max-w-md p-6 space-y-4" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="text-slate-300 text-sm">
              ...
              {deleteConfirm._count?.products > 0 && (
                <span className="text-red-400 block mt-1">This category has {deleteConfirm._count.products} product(s) — they will lose their category.</span>
              )}
            </p>
            <div className="flex gap-2 pt-1">
            ...
            {loading ? (
              ...
              categories.map((cat) => (
                <tr key={cat.id} className="hover:bg-slate-800/30 transition-colors">
                  ...
                  <td className="px-5 py-4 text-slate-400 text-sm max-w-[200px] truncate">{cat.description || '—'}</td>
