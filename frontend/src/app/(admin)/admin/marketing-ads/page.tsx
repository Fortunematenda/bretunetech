...
export default function MarketingAdsPage() {
  ...
  return (
    <div className="p-6">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-white mb-2">Marketing Ads</h1>
        <p className="text-slate-400">Create and manage promotional advertisements</p>
      </div>
      ...
      {stats && (
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
          <div className="bg-slate-900 rounded-lg p-4 border border-slate-800">
            ...
            <div className="text-sm text-slate-400">Total Ads</div>
          </div>
          <div className="bg-slate-900 rounded-lg p-4 border border-slate-800">
            <div className="text-2xl font-bold text-white">
              {stats.mostDownloaded?.downloadCount || 0}
            </div>
            <div className="text-sm text-slate-400">Most Downloaded</div>
          </div>
          <div className="bg-slate-900 rounded-lg p-4 border border-slate-800">
            <div className="text-2xl font-bold text-white">
              {stats.mostUsedTemplate ? templateLabels[stats.mostUsedTemplate] : 'N/A'}
            </div>
            <div className="text-sm text-slate-400">Most Used Template</div>
          </div>
          <div className="bg-slate-900 rounded-lg p-4 border border-slate-800">
            <div className="text-2xl font-bold text-white">{stats.recentAds.length}</div>
            <div className="text-sm text-slate-400">Recent Ads</div>
          </div>
        </div>
      )}
      ...
      <div className="flex flex-col md:flex-row gap-4 mb-6">
        <input
          ...
          className="flex-1 px-4 py-2 bg-slate-900 border border-slate-800 rounded-lg text-white focus:outline-none focus:border-blue-500"
        />
        <select
      ...
      {loading ? (
        ...
        <div className="text-center py-8 text-slate-400">No marketing ads found</div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {ads.map((ad) => (
            <div key={ad.id} className="bg-slate-900 rounded-lg border border-slate-800 overflow-hidden">
              ...
              <div className="p-4">
                ...
                <p className="text-sm text-slate-400 mb-2">{ad.headline}</p>
                <p className="text-xs text-slate-500 mb-2">{ad.product.productName}</p>
                ...
                <div className="flex gap-2">
                  <Link
                    ...
                    className="flex-1 flex items-center justify-center gap-1 px-3 py-2 bg-slate-800 hover:bg-slate-700 text-white rounded text-sm transition-colors"
                  >
