...
const ACTION_BUTTONS: { status: string; label: string; color: string }[] = [
  ...
  { status: 'INSPECTING',              label: 'Start Inspection',   color: 'bg-primary hover:bg-primary/90 text-white' },
...
export default function AdminReturnDetailPage() {
  ...
  const updateStatus = async (newStatus: string) => {
    ...
    try {
      ...
      appToast.success(`Status updated to ${statusLabel[newStatus]}`);
      fetchReturn();
    } catch {
  ...
  if (error || !ret) return (
    <div className="max-w-7xl mx-auto px-4 py-16 text-center">
      ...
      <button onClick={() => router.push('/admin/returns')} className="inline-flex items-center gap-2 text-sm text-primary">
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6 space-y-6">
      ...
      <div className="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden">
        <div className="px-6 py-5 flex flex-col sm:flex-row sm:items-center gap-5">
          <div className="w-14 h-14 rounded-2xl bg-primary/5 border-2 border-primary/20 flex items-center justify-center text-primary shrink-0">
          ...
          <div className="flex-1 min-w-0">
            ...
            <AdminPageHeader title={ret.returnNumber} />
            <div className="flex items-center gap-4 mt-2 text-sm text-gray-500 flex-wrap">
              <Link href={`/admin/orders/${ret.order?.id}`} className="flex items-center gap-1.5 hover:text-primary transition-colors">
        ...
        {availableActions.length > 0 && (
          <div className="border-t border-gray-100 px-6 py-3 bg-gray-50/50 flex items-center gap-2 flex-wrap">
            {availableActions.map((status) => {
              ...
              if (!btn) return null;
              return (
                <Button key={status} type="button" size="sm" onClick={() => updateStatus(status)} disabled={actionBusy}>
                  {btn.label}
                </Button>
              );
            })}
            <Button type="button" variant="outline" size="sm" className="ml-auto" onClick={() => setNoteModal(true)}>
              ...
              <div className="space-y-3">
                {ret.statusHistory.map((entry: any, i: number) => (
                  <div key={entry.id} className="flex items-start gap-3">
                    <div className={`w-2 h-2 rounded-full mt-1.5 shrink-0 ${i === 0 ? 'bg-primary' : 'bg-gray-300'}`} />
        ...
        <div className="space-y-5">
          ...
          <div className="bg-white border border-gray-200 rounded-2xl shadow-sm p-6">
            ...
            <div className="space-y-2">
              ...
              {ret.customer?.email && (
                <a href={`mailto:${ret.customer.email}`} className="flex items-center gap-2 text-sm text-primary hover:text-primary">
              ...
              <Link href={`/admin/customers/${ret.customer?.id}`} className="inline-flex items-center gap-1 text-xs text-primary hover:text-primary mt-1">
          ...
          <div className="bg-white border border-gray-200 rounded-2xl shadow-sm p-6">
            ...
            <Link href={`/admin/orders/${ret.order?.id}`} className="inline-flex items-center gap-1 text-xs text-primary hover:text-primary mt-3">
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="p-5 space-y-4">
                <div>
                  ...
                  <textarea value={adminNote} onChange={(e) => setAdminNote(e.target.value)} rows={3}
                    className="w-full px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg text-sm text-gray-900 focus:outline-none focus:border-primary resize-none"
              ...
              <div className="flex gap-3 px-5 py-4 border-t border-gray-200">
                ...
                <button onClick={saveNotes} className="flex-1 py-2.5 rounded-lg bg-primary hover:bg-primary/90 text-white text-sm font-semibold">Save Notes</button>
              </div>
            </div>
          </div>
        </>
