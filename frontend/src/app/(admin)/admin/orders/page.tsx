...
import { ShoppingCart, Search, RefreshCw, ChevronLeft, ChevronRight, AlertTriangle, X, Package, Clock, Trash2, Columns, CheckSquare, Eye, ExternalLink, MoreVertical } from 'lucide-react';
    ...
    <div className="space-y-6">
              ...
              <>
                ...
                <div className="absolute right-0 top-10 z-20 w-48 bg-white border border-gray-300 rounded-xl shadow-xl overflow-hidden">
                  ...
                  <div className="border-t border-gray-300 flex">
                    <button onClick={() => { const all = ALL_COLS.map(c => c.key); setVisibleCols(all); localStorage.setItem('admin_orders_cols', JSON.stringify(all)); }} className="flex-1 py-2 text-xs text-gray-500 hover:text-gray-900 hover:bg-gray-100 transition-colors">Show all</button>
                    <div className="w-px bg-gray-700" />
                    <button onClick={() => { setVisibleCols(DEFAULT_COLS); localStorage.setItem('admin_orders_cols', JSON.stringify(DEFAULT_COLS)); }} className="flex-1 py-2 text-xs text-gray-500 hover:text-gray-900 hover:bg-gray-100 transition-colors">Reset</button>
                  </div>
                </div>
              </>
            )}
      ...
      {fetchError && (
        <div className="p-4 bg-red-50 border border-red-200 rounded-xl text-red-600 text-sm">
          ...
          <p className="text-red-600/70">{fetchError}</p>
        </div>
      )}
                  ...
                  <tr key={order.id} onClick={() => router.push(`/admin/orders/${order.id}`)} className="hover:bg-gray-50 transition-colors cursor-pointer">
                    ...
                    <td className="px-5 py-4" onClick={(e) => e.stopPropagation()}>
                      <div className="relative flex justify-end">
                        ...
                        {selected?.id === order.id && !selected?.items && (
                          <>
                            ...
                            <div className={`absolute right-0 z-20 w-48 bg-white border border-gray-200 rounded-xl shadow-xl overflow-hidden ${
                              ...
                              <button
                                onClick={() => { setSelected(null); setDeleteConfirm(order); }}
          ...
          <div className="flex items-center justify-between px-5 py-3 border-t border-gray-200">
            ...
            <div className="flex items-center gap-1">
              <button onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page <= 1}
                className="p-1.5 rounded-lg text-gray-500 hover:text-gray-900 hover:bg-gray-100 disabled:opacity-30 transition-colors">
        ...
        <>
          ...
          <div className="fixed right-0 top-0 h-full w-full max-w-md bg-white border-l border-gray-200 z-50 flex flex-col shadow-2xl">
            ...
            <div className="flex items-center justify-between px-5 py-4 border-b border-gray-200">
              <div>
                <p className="text-xs text-gray-500 font-mono">{selected.orderNumber}</p>
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-5">
              ...
              <div className="flex items-center justify-between">
                <span className={`inline-flex px-2.5 py-1 text-xs font-semibold rounded-full border ${statusStyles[selected.status] || 'bg-gray-700 text-gray-500 border-gray-300'}`}>
                  {selected.status}
                </span>
                <select
              ...
              <div className="bg-white border border-gray-200 rounded-xl p-4 space-y-2">
                ...
                <p className="text-sm text-gray-900">Order #: <span className="font-mono">{selected.orderNumber}</span></p>
              ...
              <div className="bg-white border border-gray-200 rounded-xl p-4 space-y-2">
                ...
                <p className="text-sm text-gray-900">{selected.user?.firstName} {selected.user?.lastName}</p>
                <p className="text-xs text-gray-500">{selected.user?.email}</p>
                {selected.user?.phone && <p className="text-xs text-gray-500">{selected.user.phone}</p>}
              </div>
              ...
              {selected.address && (
                <div className="bg-white border border-gray-200 rounded-xl p-4 space-y-1">
                  ...
                  <p className="text-sm text-gray-700">{selected.address.street}</p>
                  <p className="text-sm text-gray-700">{selected.address.city}, {selected.address.province}</p>
                  <p className="text-sm text-gray-700">{selected.address.postalCode}</p>
                </div>
              )}

              {/* Order items */}
              <div>
                <p className="text-xs text-gray-500 uppercase tracking-wider font-semibold mb-3">Items ({selected.items?.length ?? 0})</p>
                <div className="space-y-2">
                  {(selected.items || []).map((item: any) => (
                    <div key={item.id} className="flex items-center gap-3 bg-white border border-gray-200 rounded-xl p-3">
                      <div className="w-9 h-9 bg-gray-100 rounded-lg shrink-0 overflow-hidden border border-gray-300 flex items-center justify-center">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-sm text-gray-900 truncate">{item.product?.name || item.name || 'Product'}</p>
                        <div className="flex items-center gap-2 mt-0.5">
                          <p className="text-xs text-gray-500">Qty: {item.quantity}</p>
                          {item.warehouseLocation && (
                            <span className={`inline-flex items-center gap-1 text-[10px] font-bold px-1.5 py-0.5 rounded-full ${
                              item.warehouseLocation === 'CPT' ? 'bg-green-50 text-green-600' :
                              item.warehouseLocation === 'JHB' ? 'bg-blue-50 text-blue-600' :
                      ...
                      <span className="text-sm font-semibold text-gray-900 shrink-0">{formatPrice((item.unitPrice || item.price || 0) * item.quantity)}</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="bg-white border border-gray-200 rounded-xl p-4 space-y-2">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="text-gray-900">{formatPrice(selected.subtotal || 0)}</span>
                </div>
                {selected.shippingCost > 0 && (
                  <div className="flex justify-between text-sm">
                    <span className="text-gray-500">Shipping</span>
                    <span className="text-gray-900">{formatPrice(selected.shippingCost)}</span>
                  </div>
                )}
                <div className="flex justify-between text-sm font-bold border-t border-gray-200 pt-2 mt-2">
                  <span className="text-gray-900">Total</span>
                  <span className="text-gray-900">{formatPrice(selected.totalPrice || 0)}</span>
                </div>
              </div>

              {/* Payment */}
              <div className="bg-white border border-gray-200 rounded-xl p-4 space-y-1.5">
                <p className="text-xs text-gray-500 uppercase tracking-wider font-semibold mb-2">Payment</p>
                <div className="flex justify-between text-sm">
                  <span className="text-gray-500">Method</span>
                  <span className="text-gray-900">{selected.paymentMethod || '—'}</span>
                </div>
                {selected.paymentRef && (
                  <div className="flex justify-between text-sm">
                    <span className="text-gray-500">Ref</span>
                    <span className="text-gray-900 font-mono text-xs">{selected.paymentRef}</span>
                  </div>
                )}
              </div>

              <p className="text-xs text-gray-600 text-center">Placed {formatDate(selected.createdAt)}</p>
            </div>
            ...
            <div className="px-5 py-4 border-t border-gray-200 space-y-2">
              ...
              <button
                ...
                disabled={busy}
                className="w-full flex items-center justify-center gap-2 px-4 py-2.5 bg-white hover:bg-red-50 text-red-600 text-sm font-medium rounded-lg border border-red-200 transition-colors disabled:opacity-50"
              >
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/70 backdrop-blur-sm">
          <div className="bg-white border border-red-200 rounded-2xl p-6 w-full max-w-sm shadow-2xl mx-4">
            <div className="flex items-center gap-3 mb-4">
              <div className="w-10 h-10 rounded-full bg-red-50 flex items-center justify-center shrink-0">
                <AlertTriangle className="w-5 h-5 text-red-600" />
              </div>
              <div>
                <h3 className="text-gray-900 font-bold">Delete Order</h3>
                <p className="text-gray-500 text-sm">This action cannot be undone.</p>
              </div>
            </div>
            <p className="text-gray-700 text-sm mb-1">
              Delete order <span className="font-mono text-gray-900">{deleteConfirm.orderNumber}</span>?
            </p>
            <p className="text-gray-500 text-xs mb-4">This will be recorded with your admin account.</p>
            {deleteError && <p className="text-red-600 text-xs mb-3">{deleteError}</p>}
            <div className="flex gap-3">
              <button
                onClick={() => { setDeleteConfirm(null); setDeleteError(''); }}
                className="flex-1 px-4 py-2 bg-gray-100 hover:bg-gray-700 text-gray-700 text-sm font-medium rounded-lg transition-colors"
              >
                Cancel
              </button>
              <button
