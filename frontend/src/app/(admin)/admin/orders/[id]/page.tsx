...
import {
  ...
  XCircle, Download, User, AlertCircle, FileText, ChevronRight,
  RotateCcw,
} from 'lucide-react';
...
export default function AdminOrderDetailPage() {
  ...
  const updateStatus = async (status: string) => {
    ...
    try {
      ...
      appToast.success(`Order moved to ${status}`);
      fetchOrder();
    } catch {
      appToast.error('Status update failed');
    } finally {
      setActionBusy(false);
    }
  };
  ...
  if (error || !order) return (
    <div className="max-w-7xl mx-auto px-4 py-16 text-center">
      ...
      <button onClick={() => router.push('/admin/orders')} className="inline-flex items-center gap-2 text-sm text-primary hover:text-primary">
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6 space-y-6">
      ...
      {/* Header Card */}
      <div className="bg-white border border-gray-200 rounded-xl shadow-sm p-5 sm:p-6 space-y-3">
          ...
          actions={
            <>
              {order.status === 'PENDING' && (
                <Button type="button" size="sm" onClick={() => updateStatus('PAID')} disabled={actionBusy}>
                  <CreditCard className="w-3.5 h-3.5" /> Confirm Payment
                </Button>
              )}
              {order.status === 'PAID' && (
                <Button type="button" size="sm" onClick={() => updateStatus('PROCESSING')} disabled={actionBusy}>
                  <Package className="w-3.5 h-3.5" /> Mark Processing
                </Button>
              )}
              {order.status === 'PROCESSING' && (
                <Button type="button" size="sm" onClick={() => updateStatus('SHIPPED')} disabled={actionBusy}>
                  <Truck className="w-3.5 h-3.5" /> Mark Shipped
                </Button>
              )}
              {order.status === 'SHIPPED' && (
                <Button type="button" size="sm" onClick={() => updateStatus('COMPLETED')} disabled={actionBusy}>
              ...
              return (
                ...
                >
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center border-2 transition-all ${
                    isCompleted ? 'bg-primary border-primary text-white group-hover:bg-primary/90' :
                    isCurrent  ? 'bg-white border-primary text-primary shadow-md shadow-primary/10' :
                                 'bg-white border-gray-200 text-gray-300 group-hover:border-primary/40 group-hover:text-primary'
                  }`}>
                    {isCompleted
                  ...
                  <span className={`text-[11px] font-medium text-center leading-tight transition-colors ${
                    ...
                    isCompleted ? 'text-gray-800 group-hover:text-primary' :
              ...
              <div className="text-center py-4">
                ...
                {['COMPLETED', 'SHIPPED'].includes(order.status) && (
                  <button onClick={() => router.push(`/admin/returns?search=${order.orderNumber}`)}
                    className="inline-flex items-center gap-2 px-3 py-1.5 bg-primary hover:bg-primary/90 text-white text-xs font-medium rounded-lg transition-colors">
            ...
            <div className="p-4 space-y-2">
              ...
              {order.status === 'PAID' && (
                <button onClick={() => updateStatus('PROCESSING')} disabled={actionBusy}
                  className="w-full flex items-center gap-2.5 px-4 py-2.5 bg-primary hover:bg-primary/90 text-white text-sm font-medium rounded-lg transition-colors disabled:opacity-50">
