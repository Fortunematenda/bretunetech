...
function mapDealProduct(product: any): DealProduct | null {
  ...
  const expiresAt = product.discountExpiresAt ? new Date(product.discountExpiresAt) : null;
  ...
  useEffect(() => {
    ...
    const fetchDeals = async () => {
      try {
        ...
        const deals = ((response.products || []) as any[])
          .map(mapDealProduct)
          .filter((p): p is DealProduct => Boolean(p))
